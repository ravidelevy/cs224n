...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = F.embedding(input=source_padded, weight=self.model_embeddings.source.weight)
        padded_sequence = pack_padded_sequence(X, source_lengths)
        packed_sequence, (last_hidden, last_cell) = self.encoder(padded_sequence)
        enc_hiddens = torch.permute(pad_packed_sequence(packed_sequence)[0], (1, 0, 2))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = F.embedding(input=target_padded, weight=self.model_embeddings.target.weight)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(torch.unsqueeze(dec_hidden, 1), torch.permute(enc_hiddens_proj, (0, 2, 1))))
        ### END YOUR CODE
