...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_hat = softmax(np.dot(outsideVectors, centerWordVec.T))
    loss = np.log(y_hat[outsideWordIdx]) * -1

    y = np.eye(np.shape(outsideVectors)[0])[outsideWordIdx]
    y_diff = np.array([y_hat-y]).T
    gradCenterVec = np.matmul(outsideVectors.T, y_diff)

    gradOutsideVecs = np.matmul(y_diff, np.array([centerWordVec]))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    samples = outsideVectors[indices] * -1
    samples[0] = samples[0] * -1

    ### Please use your implementation of sigmoid in here.
    sigmoid_outputs = sigmoid(np.dot(samples, centerWordVec))
    loss = np.sum(np.log(sigmoid_outputs)) * -1

    gradCenterVec = np.matmul(sigmoid_outputs - 1, samples)
    
    np_indices = np.array([indices])
    counts = np.array([(np_indices==index).sum() for index in range(max(indices)+1)])
    instances = np.array([counts[index] for index in indices])
    transparency = 1 - sigmoid_outputs
    transparency[0] = transparency[0] * -1
    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)
    gradOutsideVecs[indices] = np.dot(np.array([np.multiply(transparency, instances)]).T, np.array([centerWordVec]))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordIndexes = [word2Ind[outsideWord] for outsideWord in outsideWords]
    for outsideWordIdx in outsideWordIndexes:
        (currentLoss, currentGradCenterVecs, currentGradOutsideVectors) = \
            word2vecLossAndGradient(centerWordVector, outsideWordIdx, outsideVectors, dataset)
        loss += currentLoss
        gradCenterVecs[centerWordIdx] += np.squeeze(currentGradCenterVecs)
        gradOutsideVectors += currentGradOutsideVectors
