...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        batch = unfinished_parses[0:batch_size] if batch_size <= len(unfinished_parses) else unfinished_parses
        transitions = model.predict(batch)
        for parse, tranisition in zip(batch, transitions):
            parse.parse_step(tranisition)
        
        unfinished_parses = [parse for parse in unfinished_parses if parse.buffer != [] or len(parse.stack) > 1]
