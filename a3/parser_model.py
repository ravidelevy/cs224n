...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden_weight_tensor = torch.empty(self.embed_size * self.n_features, self.hidden_size)
        embed_to_hidden_bias_tensor = torch.empty(self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(embed_to_hidden_weight_tensor))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(embed_to_hidden_bias_tensor))

        self.dropout = nn.Dropout(p=self.dropout_prob)
        
        hidden_to_logits_weight_tensor = torch.empty(self.hidden_size, self.n_classes)
        hidden_to_logits_bias_tensor = torch.empty(self.n_classes)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(hidden_to_logits_weight_tensor))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(hidden_to_logits_bias_tensor))
        ### END YOUR CODE
    ...
    def forward(self, w):
        ...
        relu_x = torch.add(torch.matmul(x, self.embed_to_hidden_weight), self.embed_to_hidden_bias)
        relu = nn.ReLU()
        hidden_x = relu(relu_x)
        logits = torch.add(torch.matmul(hidden_x, self.hidden_to_logits_weight), self.hidden_to_logits_bias)
        ### END YOUR CODE
        return logits
